...
async def get_websocket_user(token: str) -> Optional[User]:
    ...
    try:
        # If token is passed as Bearer token, remove the Bearer prefix
        if token.startswith('Bearer '):
            token = token.split(' ')[1]
            
        payload = decode_token(token)
        if not payload:
            print(f"Invalid token payload: {token}")
            return None
            
        # Create User object with required fields
        user = User(
            id=str(payload.get('sub', '')),  # sub is the user ID
            email=payload.get('email', ''),
            role=payload.get('role', ''),
            workspace_id=str(payload.get('workspace_id', ''))
        )
        print(f"Created WebSocket user: {user.__dict__}")
        return user
